...
* Matches are now selectable too: a "Matches come from" option under
  FanPress Settings > Club lets you point player-of-the-match voting
  at your existing fixtures table (e.g. the same club plugin that
  holds your players), the same way players are selectable. With an
  external fixtures table, voting is open while the fixture is
  published. Keep "FanPress matches" to use the built-in Match Centre.
...
* Code-standards pass: full WordPress Coding Standards check back to
  0 errors / 0 warnings across the whole plugin (four findings in
  recent screens - Yoda conditions, a reserved parameter name, and
  an inlined escaping call - corrected; no behaviour change).
* Requirements and specification refreshed to reflect the recent
  builds (question categories, Owner Management, owner avatars, and
  the selectable squad source).
