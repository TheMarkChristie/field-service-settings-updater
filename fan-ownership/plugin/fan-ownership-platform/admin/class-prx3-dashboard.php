...
class PRX3_Dashboard {
	...
	public static function render() {
		...
		echo '<li>' . esc_html( sprintf( /* translators: %s revenue. */ __( 'Share revenue recorded: %s', 'fan-ownership' ), prx3_money( $revenue ) ) ) . '</li>';
		...
		foreach ( PRX3_Ballots::open_ballots() as $ballot ) {
			...
			echo '<li><strong>' . esc_html( $ballot->post_title ) . '</strong> — ' . esc_html(
				sprintf(
				/* translators: 1: voted, 2: needed, 3: closes. */
					__( '%1$d voted of %2$d needed for quorum; closes %3$s', 'fan-ownership' ),
					(int) $tallies['members'],
					$needed,
					prx3_format_datetime( get_post_meta( $ballot->ID, '_prx3_closes', true ) )
				)
			) . '</li>';
		}
		echo '</ul>';

		$videos    = wp_count_posts( 'prx3_video' )->publish ?? 0;
