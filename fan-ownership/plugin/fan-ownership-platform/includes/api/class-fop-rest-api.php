...
class FOP_REST_API {
	...
	public static function routes() {
		...
		register_rest_route( $ns, '/me', array(
			...
			'callback'            => function () {
				...
				return array(
					...
					'club'         => array( 'name' => fop_club_name(), 'primary' => fop_setting( 'club_primary', '#1a1a2e' ), 'accent' => fop_setting( 'club_accent', '#e2b007' ) ),
		...
		register_rest_route( $ns, '/me/push-token', array(
			'methods'             => 'POST',
		...
		register_rest_route( $ns, '/ballots', array(
			'methods'             => 'GET',
			'permission_callback' => array( __CLASS__, 'owner_permission' ),
			'callback'            => function () {
				$out = array();
				...
				$closed = get_posts( array( 'post_type' => 'fop_ballot', 'post_status' => 'publish', 'posts_per_page' => 20, 'no_found_rows' => true, 'meta_key' => '_fop_state', 'meta_value' => 'published' ) );
		...
		register_rest_route( $ns, '/ballots/(?P<id>\d+)/vote', array(
			'methods'             => 'POST',
			'permission_callback' => array( __CLASS__, 'owner_permission' ),
			'callback'            => function ( WP_REST_Request $request ) {
		...
		// ---- Ideas ----
		register_rest_route( $ns, '/ideas', array(
			array(
				'methods'             => 'GET',
				'permission_callback' => array( __CLASS__, 'owner_permission' ),
				'callback'            => function () {
					...
					}, get_posts( array( 'post_type' => 'fop_idea', 'post_status' => 'publish', 'posts_per_page' => 50, 'no_found_rows' => true ) ) );
				},
			),
			array(
				'methods'             => 'POST',
				...
				'callback'            => function ( WP_REST_Request $request ) {
					...
					return is_wp_error( $id ) ? $id : array( 'id' => $id, 'status' => 'pending-moderation' );
				},
			),
		) );
		register_rest_route( $ns, '/ideas/(?P<id>\d+)/support', array(
			'methods'             => 'POST',
			'permission_callback' => array( __CLASS__, 'owner_permission' ),
		...
		register_rest_route( $ns, '/questions', array(
			array(
				...
				'callback'            => function () {
					...
					}, get_posts( array( 'post_type' => 'fop_question', 'post_status' => 'publish', 'posts_per_page' => 50, 'no_found_rows' => true ) ) );
				},
			),
			array(
				'methods'             => 'POST',
		...
		register_rest_route( $ns, '/questions/(?P<id>\d+)/upvote', array(
			'methods'             => 'POST',
			'permission_callback' => array( __CLASS__, 'owner_permission' ),
		...
		register_rest_route( $ns, '/meetings/(?P<id>\d+)/rsvp', array(
			'methods'             => 'POST',
			'permission_callback' => array( __CLASS__, 'owner_permission' ),
		...
		register_rest_route( $ns, '/matches/(?P<id>\d+)/events', array(
			'methods'             => 'POST',
			'permission_callback' => array( __CLASS__, 'owner_permission' ),
			'callback'            => function ( WP_REST_Request $request ) {
		...
		register_rest_route( $ns, '/chat/(?P<room>[a-z0-9\-]+)', array(
			array(
				'methods'             => 'GET',
				'permission_callback' => array( __CLASS__, 'owner_permission' ),
		...
		register_rest_route( $ns, '/videos', array(
			...
			'permission_callback' => array( __CLASS__, 'owner_permission' ),
			'callback'            => function ( WP_REST_Request $request ) {
				$args = array( 'post_type' => 'fop_video', 'post_status' => 'publish', 'posts_per_page' => 30, 'no_found_rows' => true );
				if ( $request['type'] ) {
					$args['tax_query'] = array( array( 'taxonomy' => 'fop_video_type', 'field' => 'slug', 'terms' => sanitize_key( (string) $request['type'] ) ) );
				}
				if ( $request['search'] ) {
		...
		register_rest_route( $ns, '/videos/(?P<id>\d+)/position', array(
			'methods'             => 'POST',
			'permission_callback' => array( __CLASS__, 'owner_permission' ),
			'callback'            => function ( WP_REST_Request $request ) {
		...
		// ---- Decisions & documents ----
		register_rest_route( $ns, '/decisions', array(
			'methods'             => 'GET',
			'permission_callback' => array( __CLASS__, 'owner_permission' ),
			'callback'            => function () {
				...
				}, get_posts( array( 'post_type' => 'fop_decision', 'post_status' => 'publish', 'posts_per_page' => 100, 'no_found_rows' => true ) ) );
			},
		) );
	}
