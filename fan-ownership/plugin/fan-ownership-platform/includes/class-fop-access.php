...
class FOP_Access {
	...
	public static function can_access_board_item( $post_id, $user_id = null ) {
		...
		$recused = get_post_meta( $post_id, '_fop_recused_users', true );
		...
		if ( FOP_Board::observer_has_access( $post_id, $user_id ) ) {
			return true;
		}
		// Break-glass: full admins may enter, always audited (FO-226 AC1).
		if ( user_can( $user_id, 'manage_options' ) ) {
			FOP_Audit::log( 'board_break_glass', sprintf( 'Admin %d accessed board item %d', $user_id, $post_id ), array( 'post' => $post_id ) );
			return true;
		}
		return false;
	}
	...
	public static function guard() {
		...
		if ( is_singular( fop_board_post_types() ) ) {
		...
		if ( is_singular( fop_gated_post_types() ) || is_post_type_archive( fop_gated_post_types() ) || is_tax( array( 'fop_video_type', 'fop_document_type' ) ) ) {
			// Teaser exception: a gated item may expose its public teaser page (P36).
			if ( is_singular() && get_post_meta( get_queried_object_id(), '_fop_teaser_public', true ) ) {
	...
	public static function join_url() {
		$page_id = (int) fop_setting( 'join_page_id', 0 );
	...
	public static function hide_from_search_and_feeds( $query ) {
		...
		if ( $query->is_main_query() || $query->is_search() || $query->is_feed() ) {
			$not_in = fop_board_post_types();
			if ( ! fop_is_owner() && ( $query->is_search() || $query->is_feed() ) ) {
				$not_in = array_merge( $not_in, fop_gated_post_types() );
			}
	...
	public static function gate_content( $content ) {
		$type = get_post_type();
		if ( in_array( $type, fop_board_post_types(), true ) && ! self::can_access_board_item( get_the_ID() ) ) {
			return '';
		}
		if ( in_array( $type, fop_gated_post_types(), true ) && ! fop_is_owner() ) {
	...
	public static function gate_excerpt( $excerpt ) {
		if ( in_array( get_post_type(), array_merge( fop_gated_post_types(), fop_board_post_types() ), true ) && ! fop_is_owner() ) {
	...
	public static function track_view_activity() {
		if ( fop_is_owner() && is_singular( fop_gated_post_types() ) ) {
			fop_touch_activity();
		}
	}
}
