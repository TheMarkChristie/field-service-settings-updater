...
class PRX3_Chat {
	...
	public static function moderate( $message_id, $action, $moderator_id, $target_days = 1 ) {
		...
		$message = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}prx3_chat_messages WHERE id = %d", (int) $message_id ), ARRAY_A );
		if ( ! $message ) {
		...
		switch ( $action ) {
			case 'delete':
				...
				break;
			case 'release':
				$wpdb->update( $wpdb->prefix . 'prx3_chat_messages', array( 'held' => 0 ), array( 'id' => (int) $message_id ) );
				break;
			case 'timeout':
