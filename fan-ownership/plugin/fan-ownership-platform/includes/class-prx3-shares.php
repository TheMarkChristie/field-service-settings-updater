...
class PRX3_Shares {
	...
	public static function grant_shares( $user_id, $count, $source, $context = array() ) {
		...
		if ( $count < 1 ) {
			return new WP_Error( 'prx3_invalid_count', __( 'Invalid share count.', 'fan-ownership' ) );
		}
		// FO-106 AC3: no route exceeds the cap, combining all sources.
		if ( $current + $count > prx3_max_shares() ) {
			return new WP_Error(
				'prx3_cap',
				sprintf(
					/* translators: %d cap. */
		...
		if ( ! get_user_meta( $user_id, 'prx3_adult_confirmed', true ) ) {
			return new WP_Error( 'prx3_age', __( 'Owners must confirm they are 18 or over before holding shares.', 'fan-ownership' ) );
		}

		update_user_meta( $user_id, 'prx3_shares', $current + $count );

		// First shares: promote to owner, allocate owner number, founders check.
		if ( 0 === $current ) {
			$user = get_userdata( $user_id );
			if ( $user && ! user_can( $user, 'prx3_member' ) ) {
				$user->add_role( 'fan_owner' );
			}
			self::allocate_owner_number( $user_id );
			do_action( 'prx3_member_became_owner', $user_id );
		}

		PRX3_Register::record( $user_id, 'acquisition', $count, $source, $context );
		 ...
		 */
		do_action( 'prx3_shares_granted', $user_id, $count, $current + $count, $source );
		return true;
	}
	...
	public static function surrender_all( $user_id, $reason, $context = array() ) {
		...
		if ( $held > 0 ) {
			update_user_meta( $user_id, 'prx3_shares', 0 );
			PRX3_Register::record( $user_id, 'surrender', $held, $reason, $context );
		}
		$user = get_userdata( $user_id );
		if ( $user ) {
			$user->remove_role( 'fan_owner' );
		}
		do_action( 'prx3_shares_surrendered', $user_id, $held, $reason );
		return $held;
	}
	...
	public static function transfer_on_death( $from_user, $to_user ) {
		$held = prx3_shares( $from_user );
		if ( $held < 1 ) {
			return new WP_Error( 'prx3_none', __( 'No shares to transfer.', 'fan-ownership' ) );
		}
		...
		update_user_meta( $from_user, 'prx3_shares', 0 );
		PRX3_Register::record( $from_user, 'surrender', $held, 'deceased', array( 'to' => $to_user ) );
		return true;
	}
	...
	private static function allocate_owner_number( $user_id ) {
		global $wpdb;
		if ( get_user_meta( $user_id, 'prx3_owner_number', true ) ) {
			return;
		}
	...
	public static function owner_number( $user_id ) {
		return (int) get_user_meta( $user_id, 'prx3_owner_number', true );
	}
	...
	public static function ticket_discounts( $user_id ) {
		...
		return array(
			'matchday' => min( 100, $shares * (int) prx3_setting( 'matchday_ticket_discount_per_share', 5 ) ),
			'season'   => min( 100, $shares * (int) prx3_setting( 'season_ticket_discount_per_share', 10 ) ),
		);
	}
}
