...
class FOP_Onboarding {

	public static function init() {
		add_action( 'fop_member_became_owner', array( __CLASS__, 'start_journey' ) );
		add_action( 'fop_onboarding_step', array( __CLASS__, 'send_step' ), 10, 2 );
		add_action( 'admin_post_fop_dismiss_onboarding', array( __CLASS__, 'dismiss' ) );
	}
	...
	public static function send_step( $user_id, $step ) {
		$state = get_user_meta( $user_id, 'fop_onboarding', true );
		...
		FOP_Comms::send( $user->user_email, $steps[ $step ]['subject'], $steps[ $step ]['body'], 'governance' );
	}
	...
	public static function mark_complete( $user_id, $key ) {
		...
		if ( is_array( $state ) ) {
			...
			update_user_meta( $user_id, 'fop_onboarding', $state );
		}
	}
