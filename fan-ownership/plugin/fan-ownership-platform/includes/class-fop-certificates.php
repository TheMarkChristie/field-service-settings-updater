 ...
 * the fop_render_certificate_pdf filter). Every version is retained.
 *
...
class FOP_Certificates {

	public static function init() {
		add_action( 'fop_shares_granted', array( __CLASS__, 'issue' ), 10, 4 );
	...
	public static function issue( $user_id, $count, $total, $source ) {
		$history   = get_user_meta( $user_id, 'fop_certificates', true );
		$history   = is_array( $history ) ? $history : array();
		$verify    = strtoupper( wp_generate_password( 10, false, false ) );
		$history[] = array(
			'issued_at'    => time(),
			'shares'       => (int) $total,
			'owner_number' => FOP_Shares::owner_number( $user_id ),
			'club_name'    => fop_club_name(), // Historical identity preserved (FO-104 AC4).
			'verify_code'  => $verify,
		);
		...
		if ( $user ) {
			FOP_Comms::send(
				$user->user_email,
				sprintf( /* translators: %s club. */ __( 'Your %s owner certificate', 'fan-ownership' ), fop_club_name() ),
				sprintf(
					...
					$user->display_name,
					FOP_Shares::owner_number( $user_id ),
					$total,
					home_url( '/?fop_certificate=latest' )
				),
				'governance'
			);
		}
		// Automatic badge issue rides the same event (T24): FOP_Badges listens too.
	}

	public static function register_verify_endpoint() {
		add_rewrite_rule( '^verify-owner/([A-Z0-9]+)/?$', 'index.php?fop_verify_code=$matches[1]', 'top' );
		add_rewrite_tag( '%fop_verify_code%', '([A-Z0-9]+)' );
	}
	...
	public static function maybe_render() {
		...
		$code = get_query_var( 'fop_verify_code' );
		if ( $code ) {
			self::render_verification( strtoupper( $code ) );
		}
		if ( isset( $_GET['fop_certificate'] ) && is_user_logged_in() ) { // phpcs:ignore WordPress.Security.NonceVerification
	...
	private static function render_verification( $code ) {
		...
		echo '<h1>' . esc_html( fop_club_name() ) . ' — ' . esc_html__( 'Owner certificate verification', 'fan-ownership' ) . '</h1>';
		if ( $surrendered ) {
	...
	private static function render_certificate( $user_id ) {
		...
		echo apply_filters( 'fop_render_certificate_pdf', $html, $cert, $user ); // phpcs:ignore WordPress.Security.EscapeOutput
		exit;
	}
}
