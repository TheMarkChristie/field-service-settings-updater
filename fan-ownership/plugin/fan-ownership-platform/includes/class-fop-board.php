...
class FOP_Board {
	...
	public static function init() {
		...
		add_action( 'admin_post_fop_board_action', array( __CLASS__, 'handle_structured_action' ) );
		add_action( 'admin_post_fop_board_vote', array( __CLASS__, 'handle_internal_vote' ) );
		add_action( 'admin_post_fop_board_release', array( __CLASS__, 'handle_release' ) );
		add_action( 'admin_post_fop_board_observer', array( __CLASS__, 'handle_observer_grant' ) );
	...
	public static function open_action( $type, $ballot_id, $summary ) {
		$actions   = get_option( 'fop_board_actions', array() );
		$actions[] = array(
		...
		update_option( 'fop_board_actions', $actions, false );
		foreach ( get_users( array( 'role' => 'fop_board_member' ) ) as $director ) {
			FOP_Comms::send( $director->user_email, __( 'Board action required', 'fan-ownership' ), $summary, 'governance' );
		}
		FOP_Audit::log( 'board_action_open', $summary );
	}
	...
	public static function handle_structured_action() {
		if ( ! current_user_can( 'fop_board' ) ) {
			wp_die( esc_html__( 'Board members only.', 'fan-ownership' ) );
		}
		check_admin_referer( 'fop_board_action' );
		...
		if ( 'casting_vote' === $action['type'] ) {
			$result = FOP_Ballot_Lifecycle::resolve_tie( $action['ballot'], $choice, $reasoning );
			if ( is_wp_error( $result ) ) {
				wp_die( esc_html( $result->get_error_message() ) );
			}
		} else {
			// Failed quorum / reserved matter: decision + reasoning to register.
			FOP_Decisions::create_from_board(
		...
		update_option( 'fop_board_actions', $actions, false );
		FOP_Audit::log( 'board_action_done', $action['summary'], array( 'reasoning' => $reasoning ) );
	...
	public static function handle_internal_vote() {
		...
		check_admin_referer( 'fop_board_vote' );
		...
		if ( ! $vote_id || 'fop_board_vote' !== get_post_type( $vote_id ) || ! in_array( $position, array( 'for', 'against', 'abstain', 'recuse' ), true ) ) {
		...
		if ( ! FOP_Access::can_access_board_item( $vote_id, $user_id ) ) {
	...
	public static function recuse( $item_id, $user_id, $how ) {
		$recused   = array_map( 'intval', (array) get_post_meta( $item_id, '_fop_recused_users', true ) );
		$recused[] = (int) $user_id;
		update_post_meta( $item_id, '_fop_recused_users', array_unique( $recused ) );
		FOP_Audit::log( 'board_recusal', sprintf( 'Director %d recused from item %d (%s)', $user_id, $item_id, $how ) );
	}
	...
	private static function maybe_minute_outcome( $vote_id ) {
		...
		$chair   = (int) fop_setting( 'board_chair_id', 0 );
	...
	public static function handle_release() {
		...
		check_admin_referer( 'fop_board_release' );
		...
		$decision_id = FOP_Decisions::create_from_board( $summary, $body, $body );
		update_post_meta( $item_id, '_fop_released_as', $decision_id );
		FOP_Audit::log( 'board_release', sprintf( 'Board item %d released to register as decision %d', $item_id, $decision_id ) );
		wp_safe_redirect( admin_url( 'admin.php?page=fop-board' ) );
		exit;
	}
	...
	public static function observer_has_access( $post_id, $user_id ) {
		$grants = (array) get_post_meta( $post_id, '_fop_observers', true );
	...
	public static function handle_observer_grant() {
		...
		check_admin_referer( 'fop_board_observer' );
		$item_id  = isset( $_POST['item_id'] ) ? absint( $_POST['item_id'] ) : 0;
	...
	public static function conflicts_profile( $user ) {
		if ( ! in_array( 'fop_board_member', (array) $user->roles, true ) ) {
			return;
		}
		$conflicts = get_user_meta( $user->ID, 'fop_conflicts', true );
		echo '<h2>' . esc_html__( 'Conflicts of interest (public on the board directory)', 'fan-ownership' ) . '</h2>';
		wp_nonce_field( 'fop_conflicts', 'fop_conflicts_nonce' );
		echo '<textarea class="widefat" rows="4" name="fop_conflicts">' . esc_textarea( $conflicts ) . '</textarea>';
	}

	public static function save_conflicts( $user_id ) {
		if ( ! isset( $_POST['fop_conflicts_nonce'] ) || ! wp_verify_nonce( sanitize_key( $_POST['fop_conflicts_nonce'] ), 'fop_conflicts' ) ) {
			return;
		}
		if ( get_current_user_id() !== $user_id && ! current_user_can( 'fop_admin' ) ) {
			return;
		}
		update_user_meta( $user_id, 'fop_conflicts', isset( $_POST['fop_conflicts'] ) ? sanitize_textarea_field( wp_unslash( $_POST['fop_conflicts'] ) ) : '' );
	}
	...
	public static function watermark_vault_view() {
		if ( ! is_singular( 'fop_vault_doc' ) ) {
			return;
		}
		$post_id = get_queried_object_id();
		$user    = wp_get_current_user();
		if ( ! FOP_Access::can_access_board_item( $post_id, $user->ID ) ) {
	...
	public static function render_workspace_home() {
		...
		$actions = array_filter( get_option( 'fop_board_actions', array() ), fn( $a ) => empty( $a['done'] ) );
		echo '<div class="wrap"><h1>' . esc_html( sprintf( /* translators: %s club. */ __( '%s Board Workspace', 'fan-ownership' ), fop_club_name() ) ) . '</h1>';
		...
		foreach ( $actions as $i => $action ) {
			...
			wp_nonce_field( 'fop_board_action' );
			echo '<input type="hidden" name="action" value="fop_board_action"><input type="hidden" name="action_index" value="' . (int) $i . '">';
			echo '<p><strong>' . esc_html( $action['summary'] ) . '</strong></p>';
			if ( 'casting_vote' === $action['type'] ) {
				$options = (array) get_post_meta( $action['ballot'], '_fop_options', true );
