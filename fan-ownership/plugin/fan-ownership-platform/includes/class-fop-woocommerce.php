...
class FOP_WooCommerce {
	...
	public static function share_product_id() {
		return (int) fop_setting( 'share_product_id', 0 );
	}
	...
	public static function validate_add_to_cart( $passed, $product_id, $quantity ) {
		...
		if ( ! fop_feature_on( 'checkout' ) ) {
			wc_add_notice( __( 'The share checkout is temporarily unavailable.', 'fan-ownership' ), 'error' );
			return false;
		}
		$is_gift = ! empty( $_REQUEST['fop_gift'] ); // phpcs:ignore WordPress.Security.NonceVerification
		if ( $is_gift ) {
		...
		if ( ! FOP_Membership::is_verified( $user_id ) && ! fop_is_owner( $user_id ) ) {
	...
	public static function apply_ladder_pricing( $cart ) {
		foreach ( $cart->get_cart() as $item ) {
			...
			$is_gift = ! empty( $item['fop_gift'] );
			// Gifts price from the bottom of the ladder (recipient position unknown).
			$held  = $is_gift ? 0 : fop_shares( get_current_user_id() );
			$qty   = max( 1, (int) $item['quantity'] );
			$total = fop_ladder_total( $held, $qty );
	...
	public static function verify_order_pricing( $order, $data ) {
		foreach ( $order->get_items() as $order_item ) {
			...
			$is_gift  = (bool) $order_item->get_meta( 'fop_gift' );
			$held     = $is_gift ? 0 : fop_shares( get_current_user_id() );
			$qty      = max( 1, (int) $order_item->get_quantity() );
			$expected = fop_ladder_total( $held, $qty );
			...
			$order_item->add_meta_data( 'fop_shares_qty', $qty, true );
			$order_item->add_meta_data( 'fop_ladder_from', $held + 1, true );
			if ( $is_gift ) {
				$order_item->add_meta_data( 'fop_gift', 1, true );
				$order_item->add_meta_data( 'fop_gift_recipient', sanitize_email( $order_item->get_meta( 'fop_gift_recipient' ) ), true );
			}
		}
	}
	...
	public static function fulfil_order( $order_id ) {
		...
		if ( ! $order || $order->get_meta( '_fop_fulfilled' ) ) {
			return;
		}
		...
		foreach ( $order->get_items() as $item ) {
			...
			$qty = (int) $item->get_meta( 'fop_shares_qty' );
			$qty = $qty ? $qty : (int) $item->get_quantity();

			if ( $item->get_meta( 'fop_gift' ) ) {
				FOP_Gifts::issue( $order, $qty, (string) $item->get_meta( 'fop_gift_recipient' ) );
				continue;
			}
			if ( ! $user_id ) {
		...
		$order->update_meta_data( '_fop_fulfilled', time() );
		$order->save();
	}
	...
	private static function flag_payment_identity( $order ) {
		$fingerprint = apply_filters( 'fop_payment_identity', '', $order ); // Stripe gateway hook supplies the card fingerprint.
		$user_id     = $order->get_user_id();
		...
		if ( $existing ) {
			...
			update_user_meta( $user_id, 'fop_duplicate_suspects', array_unique( array_merge( $suspects, array_map( 'intval', $existing ) ) ) );
			FOP_Audit::log( 'duplicate_flag', sprintf( 'Payment identity match: user %d and %s', $user_id, implode( ',', $existing ) ) );
		}
	}
	...
	public static function sequential_invoice_number( $number, $order ) {
		$invoice = $order->get_meta( '_fop_invoice_no' );
		if ( $invoice ) {
			return $invoice;
		}
		if ( ! $order->is_paid() ) {
		...
		global $wpdb;
		$wpdb->query( "INSERT INTO {$wpdb->options} (option_name, option_value, autoload) VALUES ('fop_invoice_seq', '1', 'no') ON DUPLICATE KEY UPDATE option_value = option_value + 1" );
		$seq     = (int) $wpdb->get_var( "SELECT option_value FROM {$wpdb->options} WHERE option_name = 'fop_invoice_seq'" );
		$invoice = sprintf( '%s-%06d', fop_setting( 'invoice_prefix', 'PP' ), $seq );
		$order->update_meta_data( '_fop_invoice_no', $invoice );
		$order->save();
		wp_cache_delete( 'fop_invoice_seq', 'options' );
		return $invoice;
	}

	public static function note_invoice( $order_id ) {
		$order = wc_get_order( $order_id );
		if ( $order && $order->get_meta( '_fop_invoice_no' ) ) {
			echo '<p>' . esc_html(
				sprintf(
					/* translators: %s invoice number. */
					__( 'Your invoice number is %s. Invoices are kept permanently in your account.', 'fan-ownership' ),
					$order->get_meta( '_fop_invoice_no' )
				)
			) . '</p>';
		}
	}
