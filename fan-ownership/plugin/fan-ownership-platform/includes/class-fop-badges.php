 ...
 *   do_action( 'fop_award_badge', $user_id, $badge_key, $context )  — receiver
 *   apply_filters( 'fop_get_member_badges', array(), $user_id )     — provider
...
class FOP_Badges {
	...
	public static function init() {
		add_action( 'fop_shares_granted', array( __CLASS__, 'maybe_award_founder' ), 10, 4 );
		add_action( 'fop_milestone_event', array( __CLASS__, 'evaluate_milestones' ), 10, 2 );
		add_action( 'fop_retry_badge_queue', array( __CLASS__, 'retry_queue' ) );
		if ( ! wp_next_scheduled( 'fop_retry_badge_queue' ) ) {
			wp_schedule_event( time() + HOUR_IN_SECONDS, 'hourly', 'fop_retry_badge_queue' );
		}
	}
	...
	public static function award( $user_id, $badge, $context = array() ) {
		$awarded = get_user_meta( $user_id, 'fop_badges_awarded', true );
		$awarded = is_array( $awarded ) ? $awarded : array();
		if ( isset( $awarded[ $badge ] ) ) {
			return; // Idempotent.
		}
		if ( has_action( 'fop_award_badge' ) ) {
			do_action( 'fop_award_badge', $user_id, $badge, $context );
			$awarded[ $badge ] = time();
	...
	public static function retry_queue() {
		...
		update_option( 'fop_badge_queue', array(), false );
	}
	...
	public static function maybe_award_founder( $user_id, $count, $total, $source ) {
		...
		$launch = fop_setting( 'launch_moment', '' );
	 ...
	 * {key, event, threshold}. Modules fire fop_milestone_event with an
	...
	public static function evaluate_milestones( $event, $user_id ) {
		$counts           = get_user_meta( $user_id, 'fop_milestone_counts', true );
	...
	public static function member_badges( $user_id ) {
		return apply_filters( 'fop_get_member_badges', array_keys( (array) get_user_meta( $user_id, 'fop_badges_awarded', true ) ), $user_id );
	}
}
