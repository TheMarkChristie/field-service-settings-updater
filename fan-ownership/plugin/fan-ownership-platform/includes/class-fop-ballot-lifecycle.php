...
class FOP_Ballot_Lifecycle {
	...
	public static function schedule_cron() {
		if ( ! wp_next_scheduled( 'fop_ballot_tick' ) ) {
			wp_schedule_event( time() + 60, 'fop_5min', 'fop_ballot_tick' );
		}
	}

	public static function unschedule_cron() {
		wp_clear_scheduled_hook( 'fop_ballot_tick' );
	}
	...
	public static function open_due_ballots() {
		$live_cap = (int) fop_setting( 'max_live_ballots', 2 );
		$open_now = count( FOP_Ballots::open_ballots() );
		if ( $open_now >= $live_cap ) {
			return;
		}
		foreach ( FOP_Ballots::scheduled_ballots() as $ballot ) {
			if ( $open_now >= $live_cap ) {
				break;
			}
			$opens = get_post_meta( $ballot->ID, '_fop_opens', true );
	...
	public static function open_ballot( $ballot_id ) {
		...
		$active     = fop_active_owner_ids();
		$electorate = array();
		foreach ( $active as $uid ) {
			$shares = fop_shares( $uid );
			if ( $shares > 0 ) {
				$electorate[ $uid ] = $shares;
			}
		}
		update_post_meta( $ballot_id, '_fop_electorate', $electorate );
		update_post_meta( $ballot_id, '_fop_quorum_denominator', count( $electorate ) );
		update_post_meta( $ballot_id, '_fop_state', 'open' );
		update_post_meta( $ballot_id, '_fop_opened_at', fop_now() );

		FOP_Audit::log( 'ballot_open', sprintf( 'Ballot %d opened; electorate %d members', $ballot_id, count( $electorate ) ) );
	...
	public static function remind_closing_soon() {
		foreach ( FOP_Ballots::open_ballots() as $ballot ) {
			...
			if ( ! $closes || $closes - time() > DAY_IN_SECONDS || get_post_meta( $ballot->ID, '_fop_reminded', true ) ) {
				continue;
			}
			update_post_meta( $ballot->ID, '_fop_reminded', 1 );
			...
			$electorate  = (array) get_post_meta( $ballot->ID, '_fop_electorate', true );
	...
	private static function voter_ids( $ballot_id ) {
		global $wpdb;
		return array_map( 'intval', $wpdb->get_col( $wpdb->prepare( "SELECT user_id FROM {$wpdb->prefix}fop_ballot_votes WHERE ballot_id = %d", $ballot_id ) ) );
	}
	...
	public static function close_due_ballots() {
		foreach ( FOP_Ballots::open_ballots() as $ballot ) {
			$closes = get_post_meta( $ballot->ID, '_fop_closes', true );
	...
	private static function handle_failed_quorum( $ballot_id, $tallies, $needed ) {
		$is_rerun = (bool) get_post_meta( $ballot_id, '_fop_rerun_of', true );
		if ( ! $is_rerun ) {
			update_post_meta( $ballot_id, '_fop_state', 'rerun' );
			// Re-open the same ballot for another window; votes already cast stand.
			update_post_meta( $ballot_id, '_fop_rerun_of', $ballot_id );
			update_post_meta( $ballot_id, '_fop_closes', gmdate( 'Y-m-d H:i:s', time() + (int) fop_setting( 'ballot_window_days', 7 ) * DAY_IN_SECONDS ) );
			delete_post_meta( $ballot_id, '_fop_reminded' );
			update_post_meta( $ballot_id, '_fop_state', 'open' );
			FOP_Audit::log( 'ballot_rerun', sprintf( 'Ballot %d missed quorum (%d needed, %d voted) — re-running once', $ballot_id, $needed, $tallies['members'] ) );
			self::notify_electorate(
				$ballot_id,
	...
	private static function publish_result( $ballot_id, $tallies, $needed ) {
		...
		if ( $tie ) {
			...
			update_post_meta( $ballot_id, '_fop_state', 'closed' );
			$result['outcome'] = 'tie_pending_board';
			update_post_meta( $ballot_id, '_fop_result', $result );
			FOP_Board::open_action( 'casting_vote', $ballot_id, sprintf( 'Ballot %d tied — board casting vote required', $ballot_id ) );
			self::notify_electorate(
				$ballot_id,
		...
		if ( FOP_Ballots::is_constitutional( $ballot_id ) && $share < (float) fop_setting( 'constitutional_pct', 75 ) ) {
		...
		if ( 'passed' === $result['outcome'] ) {
			FOP_Decisions::create_from_ballot( $ballot_id, $options[ $top ] );
		}
		...
		$summary = 'passed' === $result['outcome']
			...
			: sprintf(
				...
				(int) fop_setting( 'constitutional_pct', 75 )
			);
		FOP_Audit::log( 'ballot_result', sprintf( 'Ballot %d published: %s', $ballot_id, $result['outcome'] ), $result );
		self::notify_electorate( $ballot_id, sprintf( /* translators: %s title. */ __( 'Result: %s', 'fan-ownership' ), get_the_title( $ballot_id ) ), $summary );
	}

	/**
	 * Board casting vote lands (called from FOP_Board): finish the tie.
	 */
	public static function resolve_tie( $ballot_id, $winning_choice, $reasoning ) {
		$result = get_post_meta( $ballot_id, '_fop_result', true );
	...
	private static function notify_electorate( $ballot_id, $subject, $body ) {
		...
		foreach ( $electorate as $uid ) {
			$user = get_userdata( $uid );
			if ( $user ) {
				FOP_Comms::send( $user->user_email, $subject, $body, 'governance' );
			}
		}
		FOP_Comms::push( $electorate, $subject, wp_strip_all_tags( $body ), 'governance', get_permalink( $ballot_id ) );
	}
}
