 ...
 *
 * FO-217, FO-219. Financial documents are fop_document posts in the
 * financial document types; the second-approval rule comes from
 * FOP_Editorial. View-only (no downloads) is enforced by never exposing
...
class FOP_Financials {

	public static function init() {
		add_action( 'fop_financial_month_check', array( __CLASS__, 'flag_missing_month' ) );
		if ( ! wp_next_scheduled( 'fop_financial_month_check' ) ) {
			wp_schedule_event( time() + DAY_IN_SECONDS, 'daily', 'fop_financial_month_check' );
		}
	...
	public static function flag_missing_month() {
		...
		update_option( 'fop_fin_flag_' . gmdate( 'Y-m' ), time(), false );
		foreach ( get_users( array( 'role__in' => array( 'fop_governance_officer', 'fop_owner_admin', 'fop_board_member', 'administrator' ) ) ) as $person ) {
			FOP_Comms::send(
				$person->user_email,
	...
	public static function viewer_endpoint() {
		add_rewrite_rule( '^owners/view-document/([0-9]+)/?$', 'index.php?fop_view_doc=$matches[1]', 'top' );
		add_rewrite_tag( '%fop_view_doc%', '([0-9]+)' );
	}

	public static function maybe_stream_inline() {
		$doc_id = absint( get_query_var( 'fop_view_doc' ) );
		if ( ! $doc_id ) {
			return;
		}
		if ( ! fop_is_owner() ) {
		...
		$attachment_id = (int) get_post_meta( $doc_id, '_fop_attachment', true );
		$path          = $attachment_id ? get_attached_file( $attachment_id ) : '';
		if ( ! $path || ! file_exists( $path ) || 'fop_document' !== get_post_type( $doc_id ) ) {
