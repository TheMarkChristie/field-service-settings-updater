...
class FOP_Comms {
	...
	public static function init() {
		add_action( 'admin_post_fop_save_prefs', array( __CLASS__, 'save_prefs' ) );
		add_action( 'admin_post_nopriv_fop_save_prefs', '__return_false' );
	}
	...
	private static function wrap( $subject, $body, $category ) {
		...
		$prefs   = esc_url( home_url( '/?fop_prefs=1' ) );
	...
	public static function user_wants( $user_id, $category, $channel ) {
		$prefs = get_user_meta( $user_id, 'fop_comms_prefs', true );
