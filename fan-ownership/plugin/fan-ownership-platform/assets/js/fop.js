 ...
 * Talks to the same fop/v1 REST API the apps use (FO-302 parity).
 * Chat uses the polling transport behind a swap-ready interface (B4).
 */
( function () {
	'use strict';
	if ( typeof fopConfig === 'undefined' ) {
		return;
	}

	function api( path, method, body ) {
		return fetch( fopConfig.restUrl + path, {
			method: method || 'GET',
			credentials: 'same-origin',
			headers: {
				'Content-Type': 'application/json',
				'X-WP-Nonce': fopConfig.nonce
			},
			body: body ? JSON.stringify( body ) : undefined
		} ).then( function ( response ) {
			return response.json().then( function ( data ) {
				if ( ! response.ok ) {
					throw new Error( data.message || fopConfig.i18n.error );
				}
				return data;
			} );
		} );
	...
	function feedback( form, message, isError ) {
		var el = form.querySelector( '.fop-feedback' );
		if ( el ) {
	...
	document.addEventListener( 'submit', function ( event ) {
		var form = event.target.closest( '[data-fop-vote]' );
		if ( ! form ) {
			return;
		}
		event.preventDefault();
		var choice = form.querySelector( 'input[name="fop_choice"]:checked' );
		if ( ! choice ) {
			return;
		}
		feedback( form, fopConfig.i18n.working, false );
		api( 'ballots/' + form.getAttribute( 'data-fop-vote' ) + '/vote', 'POST', { choice: parseInt( choice.value, 10 ) } )
			.then( function ( data ) {
	...
	document.addEventListener( 'submit', function ( event ) {
		var form = event.target.closest( '[data-fop-submit]' );
		if ( ! form ) {
			return;
		}
		event.preventDefault();
		var kind = form.getAttribute( 'data-fop-submit' );
		var payload = {
	...
	bindToggle( 'data-fop-support', 'ideas/%d/support', 'Support (%d)' );
	bindToggle( 'data-fop-upvote', 'questions/%d/upvote', 'Upvote (%d)' );
	bindToggle( 'data-fop-rsvp', 'meetings/%d/rsvp', 'RSVP (%d going)' );

	/* ---- Match centre (FO-305/306/307) ---- */
	var matchRoot = document.querySelector( '[data-fop-match]' );
	if ( matchRoot ) {
		var matchId = matchRoot.getAttribute( 'data-fop-match' );
		var room = matchRoot.getAttribute( 'data-room' );
		var lastChatId = 0;
		var player = matchRoot.querySelector( '.fop-match__player' );
		var stateEl = matchRoot.querySelector( '.fop-match__state' );
		var timeline = matchRoot.querySelector( '.fop-match__timeline' );
		var messages = matchRoot.querySelector( '.fop-chat__messages' );
		var chatForm = matchRoot.querySelector( '.fop-chat__form' );
	...
	window.addEventListener( 'online', function () { window.fopReporter.flush(); } );
	window.fopReporter.flush();
} )();
