/// The fop/v1 API client: JWT auth with refresh, typed calls for every
...
class FopApiException implements Exception {
  FopApiException(this.message, [this.statusCode]);
...
class FopApi {
  ...
  /// e.g. https://perthpanthers.example/wp-json/fop/v1
  final String baseUrl;
  static const _storage = FlutterSecureStorage();

  Future<String?> get _accessToken => _storage.read(key: 'fop_access');
  ...
  Future<void> logout() async {
    await _storage.delete(key: 'fop_access');
    await _storage.delete(key: 'fop_refresh');
  }

  Future<void> _storeTokens(Map<String, dynamic> data) async {
    await _storage.write(key: 'fop_access', value: data['access_token'] as String);
    await _storage.write(key: 'fop_refresh', value: data['refresh_token'] as String);
  }

  Future<bool> _refresh() async {
    final refresh = await _storage.read(key: 'fop_refresh');
