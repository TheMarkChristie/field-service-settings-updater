...
| `app/panthers_app/` | Flutter app skeleton (login, dashboard, Boardroom voting, Match Centre, TV library) on the plugin's `fop/v1` API |
...
2. Install companions: WooCommerce (+ Stripe gateway), bbPress, the club's
   badge plugin (implementing `fop_award_badge` / `fop_get_member_badges`), and
   a 2FA plugin (setting `fop_2fa_provider_active`).
...
4. Create pages using the shortcodes: `[fop_register]`, `[fop_share_ladder]`,
   `[fop_account]`, `[fop_dashboard]`, `[fop_ballots]`, `[fop_ideas]`,
   `[fop_questions]`, `[fop_meetings]`, `[fop_decisions]`, `[fop_videos]`,
   `[fop_board_directory]`, `[fop_redeem_gift]`, `[fop_match id="123"]`.
...
flutter run --dart-define=FOP_API=https://yourclub.example/wp-json/fop/v1
```
