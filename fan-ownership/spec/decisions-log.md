...
| P138 | Owner Account Hub (self KYC edit) | Build **3.17.0.0**: a single owner-facing hub — on **both app and web** — for an owner's **details** (bio/socials/preferences, editable), their **identity/KYC** (the six-field private record + PEP, now **viewable and editable by the owner themselves**, every change audited via `PRX3_Audit`), their **documents** (Shareholders' Agreement, ownership certificate with verify link, and published club documents), and the **forums** (browse/read/reply/new topic). New owner-scoped REST: `GET/POST /me/profile`, `GET/POST /me/identity`, `GET /me/documents`; forums reuse the existing `/forum/*` routes. App gains an **Account** tab; web gains a `[prx3_owner_hub]` shortcode and a self-installing **My Account** page assembling the existing `[prx3_profile]` + `[prx3_forum]` with a documents list. **Amends P127/P128:** those restricted identity *content moderation* to admins/board and kept identity untouchable from profile moderation — the owner editing *their own* identity record is distinct and is fully audited |
| P137 | App themes from the brand pack | Build **3.16.2.0**: the app brands itself entirely from the club brand pack — **no mobile-app settings menu is needed**. A new **public** `GET prx3/v1/config` returns the club name + full brand pack (badge, wordmark, colours, font, tagline), so the app applies the club palette, badge, club font and tagline, **including on the sign-in screen before a token exists**. Dark mode follows the device. The **in-app** look is 100% runtime-driven from what's already managed under FanPress Settings. The **launcher app name and home-screen icon** are OS/store-level and cannot be a live setting — they are per-build values read from the brand pack (`app_icon`, club name) when each club's app is built |
| P136 | App sign-in works under 2FA | Build **3.16.1.0**: the companion app can authenticate with a **WordPress Application Password**, so members can sign in even on clubs that enforce two-factor or block password-only REST logins. A new `POST prx3/v1/auth/app-login` validates the application password via core's `wp_authenticate_application_password()` and issues the same JWT pair as `/auth/login` (same sign-in throttle applied). The app tries the normal password first and **transparently falls back** to the application-password path on a 401/403, so one password field serves both; the login screen tells members to create an Application Password if their club uses two-factor. Application passwords are **per-app, revocable, and unaffected by interactive 2FA** — the WordPress-blessed credential for native apps |
