...
Members submit questions year-round; staff moderate; members upvote. The most-supported questions are answered in a **monthly Q&A video**; the rest get written answers. **Every question is put to a named audience (P126):** Board, Manager — pre-match, Manager — weekly, or Captain (the list is filterable so a club can add its own). Owners pick who the question is for on submit (web and app), staff can re-file it, the questions page and app API filter by category, and the Live Q&A presenter filters to the person on stage.
...
- **Comments** on ballots, ideas, videos, and news, **plus FanPress Chat — the platform's own native forum and social layer** (P109–P113): WhatsApp-style chat list with category chips and unread badges, bubble conversations (yours right, others left, board members in a configurable board colour with a Board tag), automated match/ballot chats embedded beside the event on its own page, activity feed with cheers, follows, searchable member directory, private messages, notifications, and @mentions with autosuggest — all behind the owner gate with one identity. Bubble colours (mine / other owners / board) are configurable under FanPress Settings → FanPress Chat. FanPress also carries **chat emails** (opt-out per member), **reply-quoting**, **staff-pinned messages**, and **per-chat mute** (P121).
- **Owner profiles (P121–P122):** every owner has a profile — bio, socials, badges, owner-since, a private-by-default share count, and an **activity percentage** (average of ballots voted ÷ held, FanPress posts in 90 days, and matches watched/listened ÷ held). Behind it sits the **private identity record**: full birth name, nationality, residence, date of birth, government registration ID, and a politically-exposed-person declaration — only birth name and nationality are public; Owner-Admins reach the rest through an audited, masked compliance lookup. Owners add a profile picture and up to five photos of themselves with explicit consent for the club's social channels; identity and photos ride the GDPR export/eraser. An owner's profile picture is used everywhere the site shows an avatar, with a clean local grey placeholder (no Gravatar) when none is uploaded (P130). **Board members and Owner-Admins manage owners from one place — Board → Owner Management (P127–P128):** find an owner, see their personal information (audited, ID masked), and moderate inappropriate community content — bio, social links, profile picture, gallery photos. Name, address, ID, PEP, and contact details are never editable there. The **Share Register** lives in the same Board menu and drills into a per-owner share record (holding, owner number, total consideration, every movement), each row linking to the owner's WP account, web profile, and Owner Management.
