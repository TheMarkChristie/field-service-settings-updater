...
| 3.17.1.0 | In-app photo upload (P138): owners set their profile photo (camera or library) and manage the five-photo consent gallery from the app. New REST `POST`/`DELETE /me/photo`, `POST /me/gallery`, `DELETE /me/gallery/{id}` (images only, WordPress core media handling; gallery capped at 5, own-photos-only removal). `profile_payload` gallery now returns `{id,url}` so the app can remove by ID; image_picker + iOS usage strings added |
