...
class Synpro_Types {
	...
	public static function render_category_image_add() {
		...
		<div class="form-field">
			<label for="synpro-category-image"><?php esc_html_e( 'Category image URL', 'syndicate-pro' ); ?></label>
			<input type="url" id="synpro-category-image" name="synpro_category_image" value="">
	...
	public static function render_category_image_edit( $term ) {
		$url = get_term_meta( $term->term_id, 'synpro_category_image', true );
		?>
		<tr class="form-field">
			<th scope="row"><label for="synpro-category-image"><?php esc_html_e( 'Category image URL', 'syndicate-pro' ); ?></label></th>
			<td>
				<input type="url" id="synpro-category-image" name="synpro_category_image" value="<?php echo esc_attr( $url ); ?>" class="regular-text">
	...
	public static function save_category_image( $term_id ) {
		if ( ! isset( $_POST['synpro_category_image'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing -- core taxonomy forms carry their own nonces, verified before these hooks fire.
			return;
		}
		$url = esc_url_raw( wp_unslash( $_POST['synpro_category_image'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.Missing
		$old = get_term_meta( $term_id, 'synpro_category_image', true );

		if ( $url !== $old ) {
			delete_term_meta( $term_id, 'synpro_category_image_id' );
		}
		if ( '' === $url ) {
			delete_term_meta( $term_id, 'synpro_category_image' );
		} else {
			update_term_meta( $term_id, 'synpro_category_image', $url );
		}
	}
	...
	public static function category_image_attachment_id( $term_id ) {
		$cached = (int) get_term_meta( $term_id, 'synpro_category_image_id', true );
		if ( $cached && get_post( $cached ) ) {
			return $cached;
		}

		$url = get_term_meta( $term_id, 'synpro_category_image', true );
		if ( ! $url || 0 !== strpos( $url, 'http' ) ) {
			return 0;
		}

		$attachment_id = Synpro_Fetcher::sideload_image( $url );
		if ( ! $attachment_id ) {
			return 0;
		}

		update_term_meta( $term_id, 'synpro_category_image_id', $attachment_id );
	...
	public static function events_archive_order( $clauses, $query ) {
		if ( is_admin() || ! $query->is_main_query() || ! $query->is_post_type_archive( 'synpro_event' ) ) {
			return $clauses;
		}
		...
		$clauses['join']   .= " LEFT JOIN {$wpdb->postmeta} synpro_start ON synpro_start.post_id = {$wpdb->posts}.ID AND synpro_start.meta_key = '_synpro_event_start'";
		$clauses['orderby'] = "(synpro_start.meta_value >= '{$now}') DESC, " .
			"CASE WHEN synpro_start.meta_value >= '{$now}' THEN synpro_start.meta_value END ASC, " .
			"COALESCE(synpro_start.meta_value, {$wpdb->posts}.post_date) DESC";
	...
	public static function add_meta_boxes() {
		add_meta_box( 'synpro_event_details', __( 'Event details', 'syndicate-pro' ), array( __CLASS__, 'render_event_box' ), 'synpro_event', 'side' );
		add_meta_box( 'synpro_podcast_details', __( 'Episode details', 'syndicate-pro' ), array( __CLASS__, 'render_podcast_box' ), 'synpro_podcast', 'side' );
		add_meta_box( 'synpro_video_details', __( 'Video details', 'syndicate-pro' ), array( __CLASS__, 'render_video_box' ), 'synpro_video', 'side' );
	}
	...
	public static function render_event_box( $post ) {
		wp_nonce_field( 'synpro_type_meta', 'synpro_type_meta_nonce' );
		$start    = get_post_meta( $post->ID, '_synpro_event_start', true );
		...
		<p>
			...
			<input type="datetime-local" id="synpro-event-start" name="synpro_event_start" value="<?php echo esc_attr( $start ); ?>" style="width:100%">
		</p>
		<p>
			<label for="synpro-event-end"><strong><?php esc_html_e( 'Ends', 'syndicate-pro' ); ?></strong></label><br>
			<input type="datetime-local" id="synpro-event-end" name="synpro_event_end" value="<?php echo esc_attr( $end ); ?>" style="width:100%">
		</p>
		<p>
			<label for="synpro-event-location"><strong><?php esc_html_e( 'Location', 'syndicate-pro' ); ?></strong></label><br>
	...
	public static function render_podcast_box( $post ) {
		wp_nonce_field( 'synpro_type_meta', 'synpro_type_meta_nonce' );
		$audio    = get_post_meta( $post->ID, '_synpro_audio_url', true );
		$duration = get_post_meta( $post->ID, '_synpro_duration', true );
		?>
		<p>
			<label for="synpro-audio-url"><strong><?php esc_html_e( 'Audio file URL (MP3)', 'syndicate-pro' ); ?></strong></label><br>
			<input type="url" id="synpro-audio-url" name="synpro_audio_url" value="<?php echo esc_attr( $audio ); ?>" style="width:100%">
		</p>
		<p>
			<label for="synpro-duration"><strong><?php esc_html_e( 'Duration', 'syndicate-pro' ); ?></strong></label><br>
			<input type="text" id="synpro-duration" name="synpro_duration" value="<?php echo esc_attr( $duration ); ?>" style="width:100%" placeholder="45:00">
		</p>
		<?php
	}
	...
	public static function render_video_box( $post ) {
		wp_nonce_field( 'synpro_type_meta', 'synpro_type_meta_nonce' );
		$video_id = get_post_meta( $post->ID, '_synpro_video_id', true );
		?>
		<p>
			<label for="synpro-video-id"><strong><?php esc_html_e( 'YouTube video ID', 'syndicate-pro' ); ?></strong></label><br>
			<input type="text" id="synpro-video-id" name="synpro_video_id" value="<?php echo esc_attr( $video_id ); ?>" style="width:100%" placeholder="dQw4w9WgXcQ">
		</p>
		<?php
	}
	...
	public static function save_meta( $post_id, $post ) {
		if ( ! isset( $_POST['synpro_type_meta_nonce'] ) || ! wp_verify_nonce( sanitize_key( $_POST['synpro_type_meta_nonce'] ), 'synpro_type_meta' ) ) {
			return;
		}
		...
		$fields = array(
			'synpro_event_start'    => array( '_synpro_event_start', 'sanitize_text_field' ),
